        ...
        if metric_results:
            ...
            for hostID, hostCollection in metric_results.items():
                ...
                for metric in hostCollection['health_metrics']:
                    ...
                    if "Cluster: Resource" in metric[0]:
                        _db[hostname]['resources'].append(metric)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Magnum RPC-JSON API Poller program for Cluster Resource Information ')

    parser.add_argument('-IP', '--address', metavar="", required=True, help='Magnum Cluster IP Address')
    parser.add_argument('-z', '--fakeit', metavar="", required=False, help='supplement some fake data')
